...
// Mock for DeliveryUsecaseInterface
type MockDeliveryUsecase struct {
	mock.Mock
}

func (m *MockDeliveryUsecase) SendMessage(ctx context.Context, msg *sarama.ConsumerMessage) error {
...
func TestMessageHandler_ConsumeClaim_Success(t *testing.T) {
	...
	claim := &mockConsumerGroupClaim{
		messages: []*sarama.ConsumerMessage{
			{Value: []byte(`{"orderID": 1}`), Topic: "delivery_key"},
		},
	}
...
func TestNewKafkaConsumer_Success(t *testing.T) {
	...
	groupID := "test-group"
	topics := []string{"delivery_key", "delivery_item"}
	mockUsecase := new(MockDeliveryUsecase)
	mockProducer := new(MockProducer)
